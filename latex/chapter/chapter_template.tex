...
%\let\oldcaption\caption %( https://tex.stackexchange.com/questions/47351/can-i-redefine-a-command-to-contain-itself ) 
%\renewcommand{\caption}[1]{\MakeSentenceCase{\oldcaption{#1}}}
...
\setchaptername{Very Long Title of Your Very Long Contribution, Which Might Span Multiple Lines (Up To Four) But Only Three In This Case}
\setNumberOfLinesForRunningTitle{3} % In this example your running title is split over two lines
\setrunningtitle{Very Long Title of Your Very \\ Long Contribution, Which Might Span \\ Multiple Lines (Up To Four) But Only Three In This Case} 
...
% Set Figures and Table Captions to "Title Case"
% \DeclareCaptionTextFormat{natocaptionformat}{\capitalizetitle{#1}} --> this doesn't work with in-text citations or equations so we have to manually adopt \capitalizetitle{} in the captions
% \captionsetup{textformat=natocaptionformat}

% The stringstrings package has a \capitalizetitle{} macro that does what you want. You need to define words that are not capitalised.
...
\begin{table}[h!]
\centering
\caption{\capitalizetitle{Security classifications. table captions Go above the table and are title case. Careful when including equations such as } $E=m\,c^2$ As They Have To Be Outside Of The Capitalize Environment.} \label{tab:classification_types}
\begin{tabular}{| c | c |}
\hline
...
\end{center}
\caption{\capitalizetitle{Figure captions should be added after the figure and are title case.}}
\label{fig:figlabel}
\end{figure}
